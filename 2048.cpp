...
float score_toplevel_move(board_t board, int move) {
	...
	if( move == 0)
		printf("Move    Up: result %f: eval'd %ld moves (%d cache hits, %d cache size) in %.2f seconds (maxdepth=%d)\n", res,
        state.moves_evaled, state.cachehits, (int)state.trans_table.size(), elapsed, state.maxdepth);
	else if (move == 1)
		printf("Move  Down: result %f: eval'd %ld moves (%d cache hits, %d cache size) in %.2f seconds (maxdepth=%d)\n", res,
			state.moves_evaled, state.cachehits, (int)state.trans_table.size(), elapsed, state.maxdepth);
	else if (move == 2)
		printf("Move  Left: result %f: eval'd %ld moves (%d cache hits, %d cache size) in %.2f seconds (maxdepth=%d)\n", res,
			state.moves_evaled, state.cachehits, (int)state.trans_table.size(), elapsed, state.maxdepth);
	else if (move == 3)
		printf("Move Right: result %f: eval'd %ld moves (%d cache hits, %d cache size) in %.2f seconds (maxdepth=%d)\n", res,
			state.moves_evaled, state.cachehits, (int)state.trans_table.size(), elapsed, state.maxdepth);
